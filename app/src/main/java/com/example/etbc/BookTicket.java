...
public class BookTicket<dbProfile> extends AppCompatActivity
{
    //location attributes
    ...
    // TableRow tb_dest,tb_pass,tb_amt,tb_journey,tb_bkBook;
    Spinner source,dest,passenger,journeyType;
    TextView amt,reason ;
    Button bkTickt;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.layout_ticketbook);
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot)
            {
                if (dataSnapshot.exists()|| dataSnapshot!=null)
                {
                  ...
                  Email = dataSnapshot.child("email").getValue().toString();
                    ...
                    Toast.makeText(BookTicket.this,"Error to fetch User Name",Toast.LENGTH_SHORT
                    ).show();
                }
            }
            @Override
        ...
        final ArrayAdapter<String> adapt= new ArrayAdapter<>(BookTicket.this, R.layout.support_simple_spinner_dropdown_item, stations);
        adapt.setDropDownViewResource(R.layout.support_simple_spinner_dropdown_item);
        source.setAdapter(adapt);
        dest.setAdapter(adapt);
        //spinner no of passenger
        ArrayAdapter<String> adapt1= new ArrayAdapter<>(BookTicket.this, R.layout.support_simple_spinner_dropdown_item, count);
        ...
        ArrayAdapter<String> adapt2= new ArrayAdapter<>(BookTicket.this, R.layout.support_simple_spinner_dropdown_item, Type);
        adapt2.setDropDownViewResource(R.layout.support_simple_spinner_dropdown_item);
        journeyType.setAdapter(adapt2);
        //check select item
        locationManager=(LocationManager)getSystemService(Context.LOCATION_SERVICE);
        //GPS IS ENABLE OR NOT
        if(!locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)){
            // wrte fun to enable gps
            onGPS();
        }
        else {
        ...
        bkTickt.setVisibility(View.INVISIBLE);
        dest.setVisibility(View.INVISIBLE);
        journeyType.setVisibility(View.INVISIBLE);
        source.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener()
        {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                sourceselet=source.getSelectedItem().toString();
                final Iterator<Map.Entry<String,List<Double>>> itr = hmLang.entrySet().iterator();
                while (itr.hasNext()){
                    Map.Entry<String, List<Double>> entry = itr.next();
                    if(sourceselet.equals(entry.getKey())) {
                        List<Double> stacode = entry.getValue();
                        m1 = stacode.get(0);
                        n1 = stacode.get(1);
                    } double totoldist = distance(lat, longt,m1,n1);
                    Toast.makeText(BookTicket.this,String.valueOf(totoldist),Toast.LENGTH_LONG).show();
                    if(totoldist<=5){
                        amt.setVisibility(View.VISIBLE);
                        passenger.setVisibility(View.VISIBLE);
                        bkTickt.setVisibility(View.VISIBLE);
                        dest.setVisibility(View.VISIBLE);
                        journeyType.setVisibility(View.VISIBLE);
                    }else{
                        amt.setVisibility(View.INVISIBLE);
                        passenger.setVisibility(View.INVISIBLE);
                        bkTickt.setVisibility(View.INVISIBLE);
        ...
        bkTickt.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v) {
                if(source.getSelectedItem().toString().compareTo(dest.getSelectedItem().toString())==0)
                {
                    Toast.makeText(BookTicket.this,"Source and Destination cannot be Same!'",Toast.LENGTH_LONG).show();
                }
                else
                {
                    calculatedate();
    ...
    public void calulateprice() {
        if (sourceselet!=null || sourceselet!="Select Station")
        {
        final Iterator<Map.Entry<String,List<Double>>> itr = hmLang.entrySet().iterator();
        while (itr.hasNext()){
            Map.Entry<String, List<Double>> entry = itr.next();
        ...
        if(source.getSelectedItem().toString().compareTo
    ...
    public void paydialog() {
        final Dialog dialog=new Dialog(BookTicket.this);
    ...
    {
        if(ActivityCompat.checkSelfPermission(BookTicket.this, Manifest.permission.ACCESS_FINE_LOCATION)
                != PackageManager.PERMISSION_GRANTED
                &&ActivityCompat.checkSelfPermission(BookTicket.this,Manifest.permission.ACCESS_COARSE_LOCATION)
                != PackageManager.PERMISSION_GRANTED)
        {
            ...
            if (locationgps != null)
            {
                ...
                //showlocation.setText("latitube" + latitude + "Longitude" + longitude);
            }
            ...
            else if (locationpassive != null)
            {
                ...
                startActivity(new Intent(BookTicket.this,User_Menu.class));
                Toast.makeText(BookTicket.this, "can't grt lovation", Toast.LENGTH_LONG).show();
            }
        }
